...
3. **Verification protocol** — **Pass:** one pass over the assembled finding list using `beagle-core:review-verification-protocol` plus the Rust delta at [review-verification-protocol](../review-verification-protocol/SKILL.md) is recorded before the Issues section is written (see **Before Submitting Findings**). One pass total, not one per finding.
...
Make **one** pass over the assembled finding list using `beagle-core:review-verification-protocol` and the [Rust delta](../review-verification-protocol/SKILL.md). Reporting a finding is `beagle-core:verification-budget` tier REVERSIBLE — cite the evidence you already have and move on. Budget: max **1** pass. Tie-break: ship a still-uncertain finding as a question or drop it, and proceed.

...
Beagle is a Codex plugin marketplace providing framework-aware code review skills and verification workflows for pre-push reviews and GitHub bot feedback handling. It contains 10 focused plugins with 83 skills and 27 commands.
...
```bash
# In Codex settings (~/.Codex/settings.json)
{
  "plugins": ["/path/to/your/beagle"]
}
```

Restart Codex after changes to reload. For skills, start a conversation using trigger keywords. For commands, run `/<plugin-name>:<command-name>` (e.g., `/beagle-core:commit-push`).

## Skills vs Commands

See [Agent Skills](https://docs.Codex.com/en/docs/agents-and-tools/agent-skills/overview) and [Slash commands](https://docs.Codex.com/en/docs/Codex/slash-commands) for general concepts.

**Skills** (`plugins/<name>/skills/` folders): Auto-loaded by Codex when relevant. Structure: `skill-name/SKILL.md` with optional `references/` folder.

**Commands** (`plugins/<name>/commands/` folders): User-invoked with `/<plugin-name>:<command>`. Single markdown file per command.

## Creating New Skills

See [Agent Skills best practices](https://docs.Codex.com/en/docs/agents-and-tools/agent-skills/best-practices) for authoring guidance.
...
2. **Bump version** - Update `version` in `.Codex-plugin/marketplace.json` and affected plugin.json files (semver)

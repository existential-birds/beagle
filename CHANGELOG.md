...
### Added
- **beagle-latex:** New plugin for LaTeX document creation, code review, and bibliography hygiene. Three skills: `latex-document` (single-file create/compile with auto-detected engine and bundled article/report/resume-ats/beamer/IEEE/book templates; ships `compile_latex.sh` and reference docs for class selection and compilation troubleshooting), `latex-code-review` (audits `.tex` for the 9 long-form anti-patterns plus encoding/float/hardcoded-dimension issues, hooks `beagle-core:review-verification-protocol`), `latex-bibliography` (BibTeX/biblatex/biber selection, DOI/arXiv `fetch_bibtex.sh`, citation hygiene checklist). Plugin README ships a `PostToolUse` hook recipe for auto-recompile on `*.tex` edits.

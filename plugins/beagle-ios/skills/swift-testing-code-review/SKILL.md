...
Complete **in order** before recording Swift Testing review findings. Load `beagle-core:review-verification-protocol` plus the [Swift / iOS delta](../review-verification-protocol/SKILL.md) **once, at review entry** for the universal review rules.
...
3. **Evidence:** Reporting a finding is `beagle-core:verification-budget` tier REVERSIBLE — cite the evidence you already have rather than re-running the protocol per finding. **Pass:** Any violated [Review Checklist](#review-checklist) item cites `[FILE:LINE]` evidence. If you report zero issues, state `Protocol applied; no Swift Testing issues` (or equivalent) in the review summary.

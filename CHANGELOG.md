...
### Added
- **beagle-core (`execution-contract`):** New internal skill (`user-invocable: false`) defining what an execution-tier model is trusted to decide and, critically, giving it a legal way to stop. Carries capability-role vocabulary (planning model vs. execution model, defined by decision authority rather than by model name), failure matching by *shape* rather than the planner's guessed string, a baseline-diffed pre-existing-red policy, a two-attempt-per-step budget, a copy-pasteable stop-and-report template, and an explicit statement that halting with a report is a success state. Plan-writing and handoff skills inline it verbatim into emitted plans.
- **beagle-core (`verification-budget`):** New internal skill (`user-invocable: false`) providing the shared verification vocabulary that review and verify skills import instead of restating their own gates. Defines reversibility-based risk tiers (REVERSIBLE gets no ceremony; IRREVERSIBLE gets a full evidence gate), the required loop-budget syntax (max passes, an observable stop condition, and a tie-break that is always *proceed and flag*), the one-echo rule, and a ban on prove-a-negative checks.
- **beagle-analysis (`quick-plan`):** New skill that produces the same bite-sized, TDD-driven implementation plan as `write-plan` but without requiring a `brainstorm-beagle` spec. It reconstructs intent from the current conversation into an Intent Brief, runs an adaptive gap check (asks the user only where intent has real holes), and fans out parallel domain-expert exploration subagents that double as the spec's *Reference Points* and *Key Decisions* — returning file maps, exact test commands, `file:line` analogs, and stack idioms/pitfalls. Reuses `write-plan`'s `plan-template.md` and full TDD discipline verbatim (single source of truth for the output format) and tightens the spike + parallel-implementation gates, since reconstructed intent carries more unverified claims than a vetted spec. Writes to `.beagle/plans/<slug>/plan.md` and offers the same `subagent-prompt` execution handoff.

### Changed
- **BREAKING — planning and verification skills recalibrated for tiered-model execution.** These skills were written when one expensive model did planning and execution alike. They now assume planning-tier models plan and review while execution-tier models execute, and they were burning tokens by over-verifying and getting stuck. Five root causes were fixed:
  - **Unbounded verification loops.** No loop in the affected skills declared a cap or a stop condition. Every remaining loop now declares max passes, an observable stop condition, and a *proceed-and-flag* tie-break. The `"Advance only when every item is honestly yes"` gate — 17 rows in `write-plan`, 12 in `quick-plan`, a double layer in `brainstorm-beagle`, three uncapped restart loops in `verify-llm-artifacts` — is gone everywhere. Criteria were preserved; only the delivery mechanism changed.
  - **Executors had no legal way to stop.** Emitted plans now open with a mandatory `## Executor Contract` block inlining `beagle-core:execution-contract`, so a blocked executor halts and reports instead of grinding. Fixes four stuck-states: literal matching against a planner-guessed failure string, an absolute "zero regressions" demand with no pre-existing-red policy, spike failure routing the executor into spec revision, and `subagent-prompt`'s `"do not stop until the final integration check passes"` plus `"no 'use your judgment' escape hatches"`.
  - **Redundant anti-confabulation ceremony.** Per-verdict echo gates defended against one long-lived context adjudicating code it never read — a much weaker risk with fresh, narrow subagent contexts. Replaced by `verification-budget`'s one-echo rule, with a staleness re-read retained before irreversible actions.
  - **Risk-tiered verification.** `verify-llm-artifacts` collapses five hard gates into one tier-based gate: full per-finding investigation only where a verdict authorizes a deletion, record-only adjudication elsewhere. `fix-llm-artifacts` keeps every protective control (stash gate, per-fix code-presence check, `git_head` staleness abort, risky-fix prompts, post-fix lint and tests) and drops only the duplicate echo and ID-lock.
  - **Duplication.** `write-plan` 462 → 181 lines, with the eight discipline essays moved to `references/planning-disciplines.md` and the duplicated task template and Key Principles echo deleted. All review criteria consolidated into `plan-reviewer.md` as the single source. `review-verification-protocol` refactored from seven drifted copies (1,848 lines) into a language-neutral core plus six thin per-language deltas (926 lines), keeping every language-specific false-positive pattern. `llm-artifacts-detection` criteria split from per-language examples, which now load only on a language match.
- **BREAKING — code review skills apply the verification protocol once per review, not once per finding.** The protocol refactor above is only half the win: the per-finding framing was embedded at ~55 call sites across every language plugin (`"completed for every reported issue"`, `"Run once per FastAPI-related finding"`, `"For every substantive finding, you can name which protocol subsection you satisfied"`). All of them now load the protocol once at review entry and apply it in a single pass over the assembled finding list. Reporting a finding is tier REVERSIBLE and earns no ceremony; only a finding whose recommended action is IRREVERSIBLE keeps the full evidence gate. Eight prove-a-negative demands ("search ALL references") became enumerated pattern lists reported as match counts. Every remaining loop declares max passes, an observable stop condition, and a proceed-and-flag tie-break. Review criteria themselves are unchanged — 41 files across all seven plugins.
- **Fixed — broken spec handoff.** `brainstorm-beagle/references/spec-template.md` and `resolve-beagle` both pointed at `docs/specs/`, while `brainstorm-beagle` writes to `.beagle/concepts/<slug>/spec.md`. `resolve-beagle` could not locate the specs it exists to resolve. Both now use the canonical path.
- **Fixed — `quick-plan` frontmatter description** was 1,033 characters against a 1,024 limit; trimmed to 645 with every trigger phrase retained.
- **Fixed — undefined `[WORKSPACE/...]` placeholder** in `quick-plan/references/fanout-brief.md` sent subagent reports to a path no skill established; now `.beagle/plans/<slug>/fanout/<area>.md`.

**Handoffs preserved unchanged:** `.beagle/concepts/<slug>/spec.md` → `write-plan` → `.beagle/concepts/<slug>/plan.md`; `quick-plan` → `.beagle/plans/<slug>/plan.md`; both → `subagent-prompt`; the spec's eight sections, slug rules, `needs-spike-before-planning` marker, and `resolve-beagle` Open Questions hook; and both artifacts JSON schemas (`.beagle/llm-artifacts-review.json` and `.beagle/llm-artifacts-verification.json`) field-for-field.

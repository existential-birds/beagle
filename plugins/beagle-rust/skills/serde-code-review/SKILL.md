...
3. **Category check vs protocol** — **Pass when:** In **one** pass over the assembled finding list, each finding's category (derive attrs, enum tagging, `flatten`, custom impl, sqlx + serde alignment) was matched against `beagle-core:review-verification-protocol` and the Rust delta at [review-verification-protocol](../review-verification-protocol/SKILL.md) (e.g. full type definition + serde attrs before “wrong representation”; confirmed edition in `Cargo.toml` before edition-2024-only findings). One pass over the list, not a fresh protocol load per finding — reporting is `beagle-core:verification-budget` tier REVERSIBLE. Budget: max **1** pass; tie-break: ship a still-uncertain finding as a question or drop it.
...
Complete **Gates (before reporting findings)** above; gate 3 is the single verification pass over the finding list, using `beagle-core:review-verification-protocol` plus the [Rust delta](../review-verification-protocol/SKILL.md).

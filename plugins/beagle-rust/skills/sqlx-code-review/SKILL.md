...
4. **Protocol** — Load `beagle-core:review-verification-protocol` plus the Rust delta at [rust-verification-protocol](../rust-verification-protocol/SKILL.md) **after** gates 1–3 and **before** final severity labels. **Pass:** One pass over the retained finding list is recorded. One pass total, not one per finding — reporting is `beagle-core:verification-budget` tier REVERSIBLE. Budget: max **1** pass; tie-break: ship a still-uncertain finding as a question or drop it.
...
Complete **Gates (evidence before severity)**, then apply `beagle-core:review-verification-protocol` plus the [Rust delta](../rust-verification-protocol/SKILL.md) in a single pass over the retained finding list.

...
Reviewer brief (dispatch as a subagent if supported, otherwise run inline):
    ...
    **Project conventions:** [path(s) to relevant AGENTS.md or CLAUDE.md]
    ...
    | Test-tier coverage | Enumerate every project-defined tier-3 (or equivalent) trigger this plan touches — entrypoint/`main`, CLI arg parsing, env-var resolution, terminal/pty/signal handling, real stdin/stdout piping, shell scripts whose contract is shell semantics, user-visible string literals whose stability is part of the contract. For each, point at the tier-3 test the plan adds, or mark the task incomplete. A shell-script credential leak is invisible to every in-process test. |
...
Record the outcome in the plan's `## Review Outcome` section: what was reviewed against, what was fixed, what was left open and why. An honest "I did not verify the tier-3 coverage claim" is worth more to a future reader than a clean-looking summary that is not true.
